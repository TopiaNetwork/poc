...
contract_1 = new web3_1.eth.Contract(abiCode);
contract_2 = new web3_2.eth.Contract(abiCode);

(async () => {
    let account_1 = (await web3_1.eth.getAccounts())[0]
    let account_2 = (await web3_2.eth.getAccounts())[0]

    const res1 = await contract_1.deploy({ data: binCode }).send({ from: account_1, gas: 800000000 })
    console.log("contract_1 address: ", res1.contractAddress)
    const res2 = await contract_2.deploy({ data: binCode }).send({ from: account_2, gas: 800000000 })
    console.log("contract_2 address: ", res2.contractAddress)

    const config = {
        address1: res1.contractAddress,
        address2: res2.contractAddress,
    }
